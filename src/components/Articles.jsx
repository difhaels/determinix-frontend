  ...
  return (
    <div>
      ...
      <div className="py-5 flex justify-center">
        <div>
          <div className="px-2 py-2">
            <div className="w-[30rem]">
              <img src={topArticles.img} alt={topArticles.title} className="w-full h-full " />
            </div>
            <div className="w-96 px-2">
              <h1 className="text-base underline text-slate-600">{topArticles.type}</h1>
              ...
              <Link to={`/showcase/${topArticles.id}`} className="text-sm text-sky-500 hover:text-sky-300">
                Read More
              </Link>
            </div>
          </div>
        ...
        <div>
          {article.map((article) => {
            return <CardArticles id={article.id} writer={article.writer} date={article.date} title={article.title} type={article.type} short={article.short} img={article.img}/>
          })}
        </div>
      </div>
